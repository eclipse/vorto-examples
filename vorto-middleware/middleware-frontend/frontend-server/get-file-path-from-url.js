...
/**
 * get the path for a file in the project directory, or reject by returning "dummy"
 *
 * @param {string} url
...
function getFilePathFromUrl(url, basePath, { pathLib = path, baseHref = '' } = {}) {
  ...
  if (!pathLib.isAbsolute(basePath)) {
    throw new Error(`${basePath} invalid path - must be absolute`);
  }
  ...
  if (
    ...
    absolutePath.endsWith(pathLib.sep) // only files can be served (not folders) 
  ) {
    return 'dummy';
  }
  return absolutePath;
